...
fn download_if_not_exist(url: &str, to: &str) {
    ...
    println!("cargo:warning=Downloading {url} to {to}");
    match ureq::get(url).call() {
        ...
        Err(e) => {
            println!("cargo:warning=Failed to download {url}: {e} — creating empty stub for compilation");
            let mut f = OpenOptions::new()
                .create_new(true)
                .write(true)
                .open(to)
                .expect("Failed to create stub file");
            f.write_all(&[]).expect("Failed to write stub");
        }
    }
}
